...
GLmol.prototype.parsePDB2 = function(str) {
   ...
   for (var i = 0; i < lines.length; i++) {
      ...
      if (recordName == 'ATOM  ' || recordName == 'HETATM') {
         ...
         var serial_str = line.substr(6, 5)
         if ( serial_str.match(/[abcdef]/gi) ){
        	 serial = parseInt(serial_str,16);
         } else {
        	 serial = parseInt(serial_str);	 
         }
         ...
         if (atom.atom == 'O3\'') { // to connect 3' end. FIXME: better way to do?
            if (currentChain != atom.chain || currentResi + 1 != atom.resi) {               
               if (currentO3) {
                  for (var j = 0; j < num; j++) {
                     var delta = -1 + 2 / (num - 1) * j;
                     if(prevOO == null) break;//Sino se rompe
...
GLmol.prototype.getResiduesById = function(atomlist, resi) {
	var ret = [];
	resi = parseInt($.trim(resi));
	for (var i in atomlist) {
		var atom = this.atoms[atomlist[i]]; if (atom == undefined) continue;

		if (resi == atom.resi) ret.push(atom.serial);
	}
	return ret;
};
