...
Biojs.SimpleFeatureViewer = Biojs.FeatureViewer.extend(
    /** @lends Biojs.SimpleFeatureViewer */
    {
        ...
        _organizeTracks: function (features) {
            ...
            forcedTracks = new Array();
            featuresInForcedTracks = new Array();
            
            $.each(features , function(index,feature){
            	
            	if (feature.track != null){
            		featuresInForcedTracks.push(feature);
            		
            		if ($.inArray(feature.track,forcedTracks) < 0){
            			forcedTracks.push(feature.track);
            		}
            ...
            for (var i = 0; i < features.length; i++) {
            	...
            	if ($.inArray(features[i],featuresInForcedTracks) >= 0){
        			continue;
        		}
                ...
                if (features[i].type != undefined) {
                    ...
                    if (features[i].track != undefined){
                    	if ($.inArray(features[i].track, forcedTracks)){
                    		forcedTracks.push(features[i].track);
                    		featuresInForcedTracks.push(features[i]);
                    	}
                    	continue;
                    } 
                } else {
            ...
            non_forced_tracks = tracks.length ;
            $.each(forcedTracks,function(index,tracknum){
            	tracks[index + non_forced_tracks ] = $.grep(featuresInForcedTracks,function(feature){
            		original = feature.track
            		feature.track = index + non_forced_tracks;            		
            		return original == tracknum;
            	});
            	
            });
            console.log(tracks)
            
            this._tracks = tracks.length + trackShapes.length + forcedTracks.length;
...
//                if(features[i].identifier == "SO:0001111"){
//                	
//                	"algo"
//                	"loco"
//                }
//                features[i].text = features[i].identifier;
        ...
        _generateLegend: function(features) {
            ...
            for (i=0; i < features.length; i++) {
                ...
                if (!found) {
                    ...
                    mykey[mykey.length-1].shape.type = features[i].type;
