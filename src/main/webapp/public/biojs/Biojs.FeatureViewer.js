...
//        printWin.close();
        ...
        _paintSlider: function() {//holder size, left and right margins of the holder, and number of amino acids
            if (this.opt.showSlider == true) {
                ...
                featureslider = this._getElementWithinTarget('uniprotFeaturePainter-slider-bar');
                ...
                featureslider.addClass("slider");
                featureslider.addClass("round");
                featureslider.addClass("slider-horizontal"); 
                
                featureslider.slider(        slider_options   );
                featureslider.slider().on("slide",function(event) {      
                	
                    myself._getElementWithinTarget('uniprotFeaturePainter-slider-values').html('Zoom - Start: '+event.value[0] + ', End: ' + event.value[1]);
                    slider_start = event.value[0];
                    slider_stop = event.value[1];
                });
                featureslider.slider().on("slideStop", function(event) {
                	
                    myself._repaint(undefined, event.value[0], event.value[1]);
                });
