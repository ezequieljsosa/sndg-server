...
*/
            ...
            if (protein.chembl) {
                ...
                $("#chembl_title").html($("#chembl_title").html() + ' <a href="https://www.ebi.ac.uk/chembl/target_report_card/' + protein.chembl.target + '">' + +'</a>');

                const assays_table = $("#assays_table");
                const activities_table = $("#activities_table");
                const assay_obj = $("#assay_obj");
                const activity_obj = $("#activity_obj");

                function setAssay(assayId){
                    assay_obj.html(
                        '<object data="https://www.ebi.ac.uk/chembl/embed/#assay_report_card/' + assayId + '/basic_information"\n' +
                        '                                width="100%" height="600px">-\n' +
                        '                        </object>'
                    );
                }
                function setActivity(compoundId){
                    const activities = '<object data="https://www.ebi.ac.uk/chembl/embed/#compound_report_card/' + compoundId + '/name_and_classification"\n' +
                        '                                width="100%" height="380px">-\n' +
                        '                        </object>\n' +
                        '                        <object data="https://www.ebi.ac.uk/chembl/embed/#compound_report_card/' +compoundId + '/molecule_features"\n' +
                        '                                width="100%" height="125px">-\n' +
                        '                        </object>';

                    activity_obj.html(activities);
                }


                const act_datatable = activities_table.DataTable({
                    "pageLength": 5,
                    "paging": true,
                    "info": true,
                    "searching": true,
                    "dom": '<"top"<"clear">>t<"bottom"irp<"clear">>f'
                });

                function loadActivities(activities){
                    act_datatable.clear().draw();

                    activities.forEach(act => {
                        const tr = $("<tr/>").appendTo(activities_table);
                        $("<td/>").appendTo(tr).html('<a>' + act.compound  + '</a>').click(()=> {
                            setActivity(act.compound)
                        });
                        $("<td/>").appendTo(tr).html(act.pchembl_value);
                        act_datatable.rows.add(tr);
                    });
                    act_datatable.columns.adjust().draw(); // Redraw the DataTable
                }


                protein.chembl.assays.forEach((assay) => {
                    const tr = $("<tr/>").appendTo(assays_table);
                    $("<td/>").appendTo(tr).html('<a>' + assay.assay + "</a>").click(()=> {
                        setAssay(assay.assay)
                        loadActivities(assay.activities);
                        setActivity(assay.activities[0].compound)
                    });
                    $("<td/>").appendTo(tr).html(assay.type);
                    $("<td/>").appendTo(tr).html(assay.description);
                });
                assays_table.dataTable({
                    "paging": protein.chembl.assays.length > 5,
                    "info": false,
                    "searching": false
                });
                loadActivities(protein.chembl.assays[0].activities);
                setAssay(protein.chembl.assays[0].assay)
                setActivity(protein.chembl.assays[0].activities[0].compound)


                const data = '<object data="https://www.ebi.ac.uk/chembl/embed/#target_report_card/' + protein.chembl.chemblid + '/approved_drugs_clinical_candidates" width="100%" height="100%"></object>';
                $("#chembl_box").parent().append($("<div/>").addClass("col-lg-12").html(data))
    ...
    <div id="patho_host_row" class="row">
        <div class="col-md-12">
            <div class="box box-solid">
                <div class="box-header">
                    <i class="fa fa-text">&#160;</i>
                    <h3 class="box-title">Reactome: Pathogen-Host interaction</h3>
                </div>
                <div class="box-body">
                    <table class="table"><tr id="reactome_tr"></tr></table>
                    <div id="diagramHolder" style="height: 500px">&#160;</div>
                    <script>
                        function onReactomeDiagramReady() {  //This function is automatically called when the widget code is ready to be used
                            var diagram = Reactome.Diagram.create({
                                "placeHolder": "diagramHolder",
                                "width": $("#diagramHolder").width() - 20,
                                "height": 500
                            });
    ...
    <div id="chembl_row" class="row">
        <div class="col-md-12">
            <div class="box box-solid">
                <div class="box-header">
                    <i class="fa fa-text">&#160;</i>
                    <h3 class="box-title" id="chembl_title">Assays and Compounds from ChEMBL25</h3>
                </div>
                <div id="chembl_box" class="box-body" style="overflow: auto;height: 600px">
                    <table id="assays_table"  class="table table-striped" >
                        <thead>
                        <tr>
                            <td>Code</td>
                            <td>Type</td>
                    ...
                    <div class="col-md-4" id="assay_obj">-</div>
                    <div class="col-md-8">
