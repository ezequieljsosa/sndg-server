<jsp:root xmlns:jsp="http://java.sun.com/JSP/Page"
    ...
    <head>
        ...
        <style type="text/css">
            ...
            th.rotate > div {
                transform: /* Magic Numbers */ translate(-15px, 0px) /* 45 is really 360 - 45 */ /*rotate(315deg); diagonal*/ rotate(270deg);
                width: 30px;
            }

            th.rotate2 > div {
                transform: /* Magic Numbers */ translate(-45px, 60px) /* 45 is really 360 - 45 */ /*rotate(315deg); diagonal*/ rotate(270deg);
                width: 100px;
            }
    ...
    <script type="text/javascript">
        ...
        function init(data) {
            ...
            if (project.trees.length > 0) {
                project.trees.forEach(x => {
                    $("<li/>").append($("<a/>", {href: "${baseURL}/variant/" + project.id + "/tree?selected=" + x.name}).html(x.name)).appendTo("#project_trees");
                });
            } else {
                $("#section_project_trees").remove();
            }
            ...
            $.filterTable.onClick = function (prop_name) {
                var prop = searchProps.filter(function (x) {
                    return x.name == prop_name
                })[0];
        ...
        function search(selectedStrains) {
            ...
            function createAlleleMenu(variant, allele, sampleAllele) {
                var elem = $("<div/>").html('<b>' + variant.contig + ":" + variant.pos.toString() + "</b> REF: " + variant.ref );
                $("<div/>").appendTo(elem).html('<b>' + sampleAllele.sample + "</b> ALT: " + allele.alt)
                $("<br/>").appendTo(elem)
                Object.keys(sampleAllele.annotations).forEach(x => {
                    $("<div/>").appendTo(elem).html(x + ": " + sampleAllele.annotations[x])
                })
                $("<a/>", {
                    href: "${baseURL}/genome/" + variant.organism + "/strain/" + project.id +
                    "?variantId=" + variant.id
                }).html("View in genome").appendTo(elem)
                return elem;
            }

            function createProtVariantMenu(variant, allele, sampleAllele) {
                var elem = $("<div/>").html('<b>' + variant.gene + ":" + allele.aa_pos.toString() + "</b> REF: " + allele.aa_ref  );
                $("<div/>").appendTo(elem).html('<b>' + sampleAllele.sample + "</b> ALT: " + allele.aa_alt);
                $("<br/>").appendTo(elem);
                if(variant.prot_ref){
                    $("<a/>", {
                        href: "${baseURL}/protein/" + variant.prot_ref + "?project=" + project.id +
                        "?variantId=" + variant.id
                    }).html("View in protein").appendTo(elem);
                }
            ...
            $.post("${baseURL}/variant/" + currentOrganism + '/', JSON.stringify(window.query)).done(function (data) {
                ...
                data.data.forEach(variant => {
                    ...
                    $("<a/>", {href: $.api.url_protein(variant.prot_ref)})
                        .html(variant.gene).appendTo($("<td/>").appendTo(tr));
                    var var_types = {};
                    ([].concat.apply([], variant.sample_alleles.map(x => x.variant_type))).forEach(x => {
                        var_types[x] = 1;
                    });
                    var_types = Object.keys(var_types).join(", ").replace("_variant", "")
                        .replace(new RegExp("_", 'g'), " ")
                    $("<td/>").html(var_types).appendTo(tr)
                    var sa1 = null;
                        ...
                        if ($("#selectVisualization").val() != "Protein") {
                            ...
                            selectedStrains.forEach(x => {
                                ...
                                if (strain_allele.length > 0) {
                                    ...
                                    var sampleAllele = strain_allele[0].samples.filter(s => s.sample == x)[0];

                                    td.click(function () {
                                        $("#modal_body").empty()
                                        $("#modal_body").append(createAlleleMenu(variant, strain_allele[0], sampleAllele));
                                        $("#myModal").modal()
                                    });

                                    if (strain_allele[0].alt.length > 1) {
                                        alt_txt = ('<b><span style="cursor:pointer" title="' + alt_txt + ' AD: ' +
                                            sampleAllele.annotations["AD"] + '" >Ins</span></b>');
                                    } else {
                                        alt_txt = ('<b><span style="cursor:pointer" title="' + 'AD: ' +
                                            sampleAllele.annotations["AD"] + '" >' + alt_txt + '</span></b>');
                                    }
                            ...
                            selectedStrains.forEach(x => {
                                ...
                                if (strain_allele.length > 0) {
                                    ...
                                    if (strain_allele[0].aa_alt.length > 1) {
                                        ...
                                        alt_txt = ('<b><span style="cursor:pointer">'+alt_txt+'</span></b>');
                                    }
                                    var td = $("<td/>");
                                    var sampleAllele = strain_allele[0].samples.filter(s => s.sample == x)[0];
                                    ...
                                    td.click(function () {
                                        $("#modal_body").empty()
                                        $("#modal_body").append(createProtVariantMenu(variant, strain_allele[0], sampleAllele));
                                        $("#myModal").modal()
                                    });
                                    sa1 = strain_allele[0];
                                } else {
        ...
        function showHeader(selectedStrains) {
            ...
            $('<th/>', {height: 70, width: 180}).html('<div><span>Pos</span></div>').appendTo(header);
            $('<th/>').html('').appendTo(header2);

            $('<th/>', {height: 70, width: 90}).html('<div><span>Gene</span></div>').appendTo(header);
            $('<th/>').html('').addClass("rotate").appendTo(header2);

            //$("<th/>",{title:"Reported in database"}).html("Reported").appendTo(header);
            //$('<th/>').html('').appendTo(header2)

            $('<th/>', {height: 70, width: 140}).html('<div><span>Type</span></div>').appendTo(header);
            $('<th/>').html('').addClass("rotate").appendTo(header2);

            if ($("#selectVisualization").val() != "Protein") {
                $('<th/>', {
                    width: 30,
                    title: "Nucleotide Reference"
                }).addClass("rotate2").html('<div><span>Nucl Reference</span></div>').appendTo(header);
                $('<th/>').html('').addClass("rotate").appendTo(header2);


                $('<th/>', {
                    width: 20 * selectedStrains.length,
                    colspan: selectedStrains.length,
                    title: "Nucleotide Alternative"
                }).html('Alt').appendTo(header).css("background-color", "grey").css("text-align", "center");
                ...
                selectedStrains.forEach(x => {
                    ...
                    $('<th/>', {
                        width: 20,
                        title: x
                    }).html("<div><span>" + x.toString() + '</span></div>').addClass("rotate").appendTo(header2);
                });
            }

            if ($("#selectVisualization").val() != "Nucleotide") {
                $('<th/>', {title: "Aminoacid Position"}).html('<div><span>AA Pos</span></div>').addClass("rotate2").appendTo(header).css("width", "40px")
                $('<th/>').html('').addClass("rotate2").appendTo(header2);

                $('<th/>', {width: 30, title: "Amnoacid Reference"}).addClass("rotate2")
                    .html('<div><span>AA Reference</span></div>').css("width", "40px").appendTo(header) //.addClass("verticalTableHeader").css("padding-right",35).css("padding-bottom",10)
                $('<th/>').html('').addClass("rotate").appendTo(header2)

                $('<th/>', {
                    colspan: selectedStrains.length,
                    width: 25 * selectedStrains.length,
                    title: "Strain Aminoacid Alternative"
                }).html('Aminoacid Alternative').appendTo(header).css("background-color", "grey").css("text-align", "center")
                ...
                selectedStrains.forEach(x => {
                    $('<th/>', {
                        width: 20,
                        title: x
                    }).addClass("rotate").html("<div><span>" + x.toString() + "</span></div>").appendTo(header2);
                });
            }
            ...
            if ($("#selectVisualization").val() != "Protein") {
                ...
                $('<th/>', {
                    colspan: selectedStrains.length,
                    width: 50 * selectedStrains.length,
                    title: "Allele Depth"
                }).html('Allele Depth')
            ...
            */
            $('<th/>').addClass("rotate").html('').appendTo(header2)
            $('<th/>').addClass("rotate").html('').appendTo(header)
            ...
            <div class="box">
                ...
                <div class="box-body">
                    <table class="table table-striped" id="strains_table">
                        ...
                        <tbody id="strains_table_body"></tbody>
                    </table>
                </div>
            </div>
        </section>
        <section id="section_project_trees" class="col-lg-3">
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">Tree list</h3>
                </div>
                <div class="box-body">
                    <ul id="project_trees">
                ...
                <div class="box-body">
                    ...
                    <table id="restrictionTable">
                        <tr>
                            ...
                            <td>
                                <button onclick="addRestriction()">Add
                                    Restriction
                                </button>
                            </td>
                        </tr>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="input-group" style="width: 100%">
						...
						<span class="input-group-addon" id="keyword_search_icon"><i
                                class="fa fa-search">&#160;</i></span> <select type="search"
                                                                               class="form-control" id="search_select2"
                                                                               style="width: 99%">
                        <option></option>
                    </select>
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Variant Data</h4>
                </div>
