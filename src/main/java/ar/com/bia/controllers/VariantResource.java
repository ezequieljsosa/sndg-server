...
public class VariantResource {
	...
	public String allGeneProds(@PathVariable("project_id") String projectId, Model model, Principal principal)
		...
		model.addAttribute("strains", mapperJson.writeValueAsString(genome.projectStrains(projectId) ));
	...
	@RequestMapping(value = "/{genome_id}/tree", method = RequestMethod.GET, produces = MediaType.TEXT_HTML_VALUE)
	public String tree(@PathVariable("genome_id") String genomeId, Model model, Principal principal)
			throws IOException {
		...
		String tree = new String(Files.readAllBytes(Paths.get("/data/projects/Staphylococcus/annotation/tree/outgrouped2.newick"))).replace("\n","");
		model.addAttribute("tree", tree );
		model.addAttribute("strains", mapperJson.writeValueAsString(new String[]{"15-6324_S3_L001","16-12-16-138_S2_L001","1109_S1_L001","2003_S4_L001"}));
...
//		}).collect(Collectors.toList());
	...
	private Map<String, GeneProductDoc> metagenome(String genomeId, String keywords_filter) {
		...
		// lista
		List<ObjectId> genomes = ref_genome.getStrains();
	 ...
	 * 
	 * @param genomeId:
	 *            Identificador de genoma del organismo
	 * @param proteinId:
	 *            Identificador de proteina de referencia
